...
from chrispnet.modules.attention.attention_nemo import (
    MultiHeadAttention,
...
class ConformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.conv = ConformerConvolution(
            d_model=d_model, kernel_size=conv_kernel_size, norm_type=conv_norm_type
        )
...
if __name__ == "__main__":
    ...
    # model = VGGExtractor(input_dim)
    # model = CNNExtractor(input_dim, output_dim)
    model = ConformerLayer(
        d_model,
        d_ff,
        self_attention_model="rel_pos",
        n_heads=4,
    ...
    T = 1600
    D_feats = 512
    feats = torch.randn(B, T, D_feats)
    # feats = torch.randn(B, D_feats, T)
    # feats_mask = torch.randn(B)
    # feats_mask = torch.Tensor([1600, 324, 1, 666])
    feats_mask = torch.Tensor([1600])
    print(feats.shape, feats_mask.shape)  # torch.Size([1, 400, 512]) torch.Size([1])
    vgg_output, vgg_mask = model.forward(feats, feats_mask)
    # print(vgg_output.shape, np.array(vgg_mask))  # torch.Size([1, 400, 640]) [1600.]
