...
spec = importlib.util.spec_from_file_location(
    "package_info", "chrispnet/package_info.py"
)
...
extras_require["test"] = list(chain([extras_require["core"],
                                     extras_require["common"]]))
extras_require["asr"] = list(
    chain([extras_require["asr"],
           extras_require["core"],
           extras_require["common"]])
)
...
class StyleCommand(distutils_cmd.Command):
    ...
    def _isort(self, scope, check):
        return self.__call_checker(
            base_command=self.__ISORT_BASE.split(), scope=scope, check=check,
        )
